	...
	for(k=1;k<=27;k++){
		...
		if(auto_opt==1 && cellNEI[ii][k]==ii){
			...
			d1x=x1[i];
        		...
        		//if(rp>=rp_min && rp<=rp_max && s_pi>=pi_min && s_pi<=pi_max) { 
  			ic=floor(log10(rp/pow(10,rp_min))/drp  +1.0);
         		//ic=floor((rp-rp_min)/drp +1.0);
...
printf("# %s %s ln(r_p)=(%6.2f,%6.2f) r_pi=(%6.2f,%6.2f) %d r_p bins %d r_pi bins L_x=%5.2fh^-1Mpc L_y=%5.2fh^-1Mpc L_z=%5.2fh^-1Mpc %d  %d Halos\n",file1,file2,rp_min,rp_max,pi_min,pi_max,Nbin_rp,Nbin_pi,Lx,Ly,Lz,N1,N2);
 ...
 for (j=1;j<=Nbin_pi;j++)
   for(i=1;i<=Nbin_rp;i++)
     fprintf(fp,"%10.6f %10.6f %e\n",pow(10,rp_cen[i]),pi_cen[j],Npair[i][j]);
 fclose(fp);
